...
\eeqn
where $A_i$ are $(0,1)$-forms on $\C^3$, and $P(X,Y) = P(X-Y)$ is as in equation \eqref{eqn:propagtorCS}.
Plugging in $A = x_1 \d \br z$ and $B = y_2 \d \br w$ this integral becomes $\int_{z,w} \d z \, \d w \,  \del_z \del_w I(z,w)$ where
\beqn
I (z,w) \define \frac{\br z - \br w}{4 \pi^3} \int_{\C^2 \times \C^2} \d^4 x \d^4 y \frac{[\br x \br y] x_1 y_2}{\|x\|^4 (|z-w|^2 + \|x-y\|^2)^3 \|y\|^4} .
\eeqn
...
\beqn
I(z,w) = \frac{2 \pi}{z-w} \int_{0}^1 \d t \int_0^1 \d s \frac{(1-t)(1-s)}{(1-ts)^3} .
\eeqn
The integral over $t,s$ gives $\pi/(z-w)$ thus leading to the expression in the bulk of the paper.
...
\subsection{The central term from Kodaira--Spencer theory}
...
\beqn
I (z,w) \define \frac{(\br z - \br w)^2}{\#} \int_{\C^2_x \times \C^2_y} \d^4 x \d^4 y \frac{[\br x \br y] x_1 y_2}{\|x\|^4 (|z-w|^2 + \|x-y\|^2)^4 \|y\|^4} .
\eeqn
We compute $I(z,w)$ as a function of the difference $z-w$.
...
Using Feynman's trick we have
\begin{multline}
\int_{\C^2} \d^4 y \frac{[\br x \br y] y_2}{(|z-w|^2 + \|x-y\|^2)^4 \|y\|^4} \\ = \frac{??}{??} \int_0^1 \d t \, t^3 (1-t) \int_{\C^2} \d^4 y \frac{[\br x \br y] y_2}{\left(t |z-w|^2 + t \|x-y\|^2 + (1-t)\|y\|^2 \right)^6} .
\end{multline}
Introduce the new variable $\til y = y - t x$.
The the right hand side becomes
\beqn
??\int_0^1 \d t \, t^2 (1-t) \int_{\C^2} \d^4 \til y \frac{[\br x (\br {\til y} + t \br x)] (y_2+t x_2)}{\left(\|\til y\|^2 + t(1-t)\|x\|^2 + t |z-w|^2 \right)^5} 
\eeqn
The $U(1) \times U(1)$ invariant part of the numerator is $\br x_1 |\til y_2|^2$.
After computing the residue along both the $\til y_1$ and $\til y_2$ directions the integral becomes
\beqn
\# \br x_1 \int_{0}^1 \d t \, t^3 (1-t) \int_{(0,\infty) \times (0,\infty)} \d^2 \rho \,\frac{\rho_2}{(\rho_1 + \rho_2 + t(1-t)\|x\|^2 + t |z-w|^2)^6} .
\eeqn
Performing the integration over $(0,\infty) \times (0 , \infty)$ we obtain 
\beqn
\#\br x_1 \int_0^1 \frac{1-t}{(|z-w|^2 + (1-t)\|x\|^2)^3} %= \frac{(-2 \pi \im)^2}{2} \br x_1 \left(- \frac{1}{\|x\|^2 (|z-w|^2 + \|x\|^2)} + \cdots\right) .
\eeqn
...
\beqn
\frac{\#}{|z-w|^4} \int_{0}^1 \d t \, \frac{(1-t)(1-s)}{(1-ts)^3}  
\eeqn
and hence
\beqn
I(z,w) = \frac{\#}{(z-w)^2} .
