...
In the type IIB theory on $\C^3 \times K3$ we consider a D1--D5 system where $N_1$ D1 branes wrap 
\[
\C \times 0 \times \{x\} \subset \C \times \C^2 \times K3 
\]
for some points $x \in K3$, and $N_5$ D5 branes wrap 
\[
...
Upon compactification, the D3 system becomes a system of $B$-type branes in this extended version of Kodaira--Spencer theory. \textcolor{red}{Did we mention the extension of KS explicitly before? We should probably do so earlier...}
...
Explicitly, if $F = f \eta + \br f \br \eta + f^a \eta_a$ for $f, \br f, f_a$ complex numbers, then $N = f \br f + f^{a} f^{b} h_{ab}$ where $h_{ab}$ is the fixed non-degenerate symmetric pairing. As usual $N = N_1(N_5-1) \simeq_{N_1, N_5 \rightarrow \infty} N_1 N_5$. \textcolor{red}{im pretty sure about the -1 but i want to double-check.}
...
Recall that in the case of ordinary Kodaira--Spencer theory on $\C^3$, it was shown in \cite{CostelloGaiotto} that the backreaction of B-branes along $\C \subset\C^3$ deformed the complex structure on $\C^3 \setminus \C$ to the {\em deformed conifold} $SL_2(\C)$. 
...
Following \cite{CostelloGaiotto}, the backreaction of this system of branes introduces a twisted supergravity field \textcolor{red}{What is the bar on top of PV?}
\[
...
\label{eqn:mcbr}
	...
	\end{split}
\eeqn
For quantization we will also impose the standard gauge fixing condition that $\dbar^\ast \mu_{BR}(\eta_a) = 0$ in terms of the usual codifferential $\dbar^{\ast}$. 	
There is a unique solution to the above equations given by
\beqn
\mu_{BR} = \frac{\eps^{ij} \br w_i \d \br w_j}{|w|^4} \partial_z \otimes F .
\eeqn
Note that this field is of the form $\mu_{BR,0} \otimes F$ where $\mu_{BR,0}$ is the Beltrami differential which gives rise to the deformed conifold \cite{CostelloGaiotto}---all of the dependence on the parameters $\eta, \br \eta, \eta_a$ is in the cohomology class~$F$.
