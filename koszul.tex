...
Let us give a heuristic for the context in which Koszul duality appears in field theory. 
...
\]
In \S \ref{s:algcoupling}, we will elaborate further on this definition. 
In \S \ref{s:lagcoupling}, we will explain the relationship of this algebraic notion of a coupling more the possibly more standard description in terms of local Lagrangians.
...
From this observation, the Koszul dual has the following interpretation: $\cA^!$ is the algebra of operators on the {\em universal} line defect supported on $\RR \times \{x\}$. 
...
This means that OPE's are independent of the coordinate $t$.
...
If we assume that the theory is also translation invariant along $M$ then there is also an equivalence of algebras $\cA_x \simeq \cA_{x'}$ for $x, x' \in M$. 
...
\brian{should we call this a line operator?}
...
We spell out an explicit example. 
...
The key to this interpretation is the following result.
...
\alpha \in \cA \otimes \cB 
\]
satisfying $(\ep \otimes \id_B) (\alpha) = 0 \in \cB$.
...
This means that the coupled system corresponding to $\alpha$ leaves the vacuum unchanged. \brian{Can we say this better?}
...
Also, observe that while the one-form field $\psi_t$ couples to a scalar $J[k]$ along, the field $\phi$ couples to a one-form $K^{(1)}[\ell] = K_t [\ell] \d t$ (and vice versa for their counterparts). 
