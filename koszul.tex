...
Before we move on to the explanation of why Koszul duality appears in physics, we compute the Koszul dual of one more algebra. The algebra in question naturally arises in the context of interacting field theories, namely as the algebra of local operators of the B-twist of a $\cN = (2,2)$ Landau-Ginzburg theory with target $V^{\vee}$ and superpotential $W: V^{\vee} \rightarrow \CC$.
...
The variable $x_i$ is even parity and $\psi^j$ is odd parity; we will furthermore take a $\ZZ$-grading where $x_i$ is degree zero and $\psi^j$ is degree $-1$. These operators have a physical origin as the $Q_B$-closed scalar and fermionic components of chiral multiplets valued in $V^{\vee}$. \natalie{right? say anything here about polyvector fields on $V^{\vee}$?}
...
It is a straightforward exercise in spectral sequences to prove that even for a more general superpotential $W$ the result still holds, and $\cA^! \simeq \cA$. We will spare the physics-oriented reader this unpleasantness. 
...
We return to Example \ref{sec:gaugealg}, which clearly had some relationship to gauge theory, and finally flesh out the physics behind it. 
...
The formula is
\[
Q_0 = \sum_{n +m \geq 0} \left(f_{ab}^c \partial_z^m \fc^a \partial_z^n \fc^b \frac{\partial}{\partial \partial_z^{n+m}\fc^c} + f_a^{bc} \partial_{z}^m \fc^a \partial_z^n B_b \frac{\partial}{\partial \partial_z^{n+m} B_c}\right) .
\]
Here, $\{f^{bc}_a\}$ denotes the contragradient structure constants of the Lie algebra, which satisfy $[e_a, e^b] = f_a^{bc} e_c$ where $\{e^a\}$ is a dual basis for $\fg^*$. 
...
As alluded to, the BRST complex of local operators is equivalent to the Chevalley--Eilenberg complex for the graded Lie algebra $\fg[\ep][[z]]$:
\[
