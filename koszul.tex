...
\natalie{comm/lie as a classical limit of assoc/assoc?}
...
Thus, to first-order, topological descent gives BRST invariance of the Lagrangian.
At the quantum level, the Lagrangian gets upgraded to a path ordered exponential. BRST invariance to all perturbative orders will force the coupling to satisfy the Maurer--Cartan equation. 
...
\natalie{We also wanted to explain how the MC equation could take the form of a MC eqn for an $A_{\infty}$ algebra. Where do we want these discussions to go?}
...
By the above discussion, we see that $\alpha$ is an algebraic coupling, meaning it satisfies the Maurer--Cartan equation for $\cA \otimes \cB$, if and only if $\int_S \alpha^{(1)}$ is BRST invariant at the perturbative quantum level.

\subsection{One-dimensional gauge theory}
\label{sec:gaugetheory}

We return to Example \ref{sec:gaugealg} which clearly had some relationship to gauge theory. 
