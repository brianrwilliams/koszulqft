...
\]
is a map of cochain complexes and induces an isomorphism on cohomology. \natalie{we need to write it explicitly. that these would be quasi-iso is non-obvious to physicists}We call such a map a \textit{quasi-isomorphism}. We will be interested in identifying Koszul dual algebras up to quasi-isomorphism, as ultimately the physical operator algebras of interest come from passing to cohomology in the BRST framework. \natalie{physically we should expand on this point more, perhaps in the examples? if so, make a note here about that.}
...
Observe \natalie{should just add the computation I think...} that the zeroth cohomology $H^0$ of this algebra is precisely the Jacobian ring of~$W$
\[
\CC[x_i] \, / \, (\partial_i W) .
\]
