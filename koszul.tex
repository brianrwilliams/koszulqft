...
\def\mod{{\rm mod}}
\newcommand{\RHom}{\mathbb{R}\op{Hom}}
...
We will often work in the generality where $\cA$ is a differentially graded (dg) algebra meaning that it is $\ZZ$ graded and equipped with a square-zero, degree one operator $\d : \cA \to \cA[1]$ such that 
\[
...
Physically speaking, one should think about $\cA$ as being the algebra of BRST operators of a system of topological quantum mechanics \brian{best way to be gentle here?}. 
...
Given $M,N \in \cA-\mod$ we denote by $\RHom_\cA(M,N)$ the cochain complex of homomorphisms. 
\brian{gentle way to define this?}

An {\em augmentation} is a map of algebras 
\[
\ep \colon \cA \to \CC .
\]
In other words, an augmentation is the data of an $\cA$-module structure on the one-dimensional module which we denote $\CC_\ep$. 
