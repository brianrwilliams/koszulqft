...
\]
Observe that, employing some identities of q-Pochhammer symbols, the characters of these two Koszul dual algebras are related by $\chi_0 (q,s) = (\chi_\infty (q, s))^{-1}$.  The property $\chi_0(q, s)\chi_{\infty}(q, s) = 1$ is expected for characters of Koszul dual algebras. 
%It would be interesting to understand how Koszul duality acts on more general indices, for which, by standard cohomological arguments, it is generally sufficient to study free theories \footnote{We thank K. Costello for a discussion of this point.}.
