...
We turn to an example in three-dimensions of a free field theory on $\RR^3$.
This theory is equivalent to the twist of the $3d$ $\cN=2$ chiral multiplet. 
The input data is simply a complex vector space $V$.
...
We move towards a description of the algebra of local operators at $z = t = 0$ up to gauge equivalence.
...
In terms of the two form field $\d \wedge \psi$ there is a one-form gauge symmetry by the field $\d z \wedge \chi$. 
...
Suppose $V$ transforms according to some $G_f$ representation, where $G_f$ is the flavor group. 
...
The character of the space of local operators $\cA_0$ can be written in terms of fugacities $q$ and $u$ where $q$ is the fugacity of the $\U(1)$ conformal symmetry and $u$ is a vector in the maximal torus $T_f$ of the flavor group $G_f$. 
\[
\chi_0(q , s) = \frac{1}{|{\rm Weyl}(G)|} \oint_{s \in T_f} \frac{\d s}{2 \pi i s} \prod_{\alpha \in {\rm wt}(V)} (1-s_\alpha) \frac{(q s^{-1}_\alpha ; q)_\infty}{(s_\alpha ; q)_\infty} .
\]

Suppose $\U(1) \to G_f$ is a cocharacter which acts on $V$ by weight $+1$. 
Then, we can break $\U(1)_q \times G_f$ to $\U(1)_q \times \U(1)_s$ and the resulting character becomes
\[
\chi_0 (q,s) = \prod_{n \geq 0} \frac{1 - s^{-1} q^{n+1}}{1 - s q^n} .
\]
...
\[
\varphi_0(t,r) \in C^\infty(\RR \times \RR_{>0} , V) ; \quad \psi_0 (t,r) \in \Omega^1 (\RR \times \RR_{>0}, V^*) 
\] 
with action $\int_{\RR\times \RR_{>0}} \varphi_0 \d \psi_0$.
...
More accurately what we should do is expand the fields $\varphi, \psi$ into their Fourier modes around the circle.
...
\begin{align*}
\varphi(t,z) & = \sum_{n \in \ZZ} \varphi_n (t,r) e^{2 \pi i n \theta} \\
\psi(t,z) & = \frac{1}{2 \pi i z} \sum_{n \in \ZZ} \psi_n(t,\theta) e^{2 \pi i n \theta} .
\end{align*}

If we expand the free action, we see that the theory on $\RR \times \RR_{>0}$ has as its fields 
\[
\{\varphi_n(t,r)\}_{n \in \ZZ} \in C^\infty(\RR \times \RR_{>0} , V) ; \quad \{\psi_n (t,r)\}_{n \in \ZZ} \in \Omega^1 (\RR \times \RR_{>0}, V^*) .
\] 
with free action
\[
\sum_{n \in \ZZ} \int_{\RR \times \RR_{>0}} \bigg(\varphi_n \d \psi_{-n} + n \varphi_n \psi_{-n} (r^{-1} \d r)  \bigg)
\]
...
\varphi_n (t, 0) = 0 ; \quad \psi_{n+1} (t,0) = 0 \quad {\rm for} \; n < 0 .
\end{equation}
...
We observe that the corresponding operators at $t = r = 0$ of this dimensionally reduced theory agree precisely with the local operators $\cA_0$ we found from the three-dimensional point of view above. 
Indeed, they are freely generated, up to gauge equivalence, by the even local operators $\{\varphi_n\}_{n \geq 0}$ and the odd local operators $\{\chi_n\}_{n > 0}$. 
In the notation of the previous section $\varphi_n$ corresponds to a scalar multiple of $\partial_z^n \varphi$ and $\chi_n$ a scalar multiple of~$\partial^{n-1}_z \chi$. 

\subsection*{Another boundary condition} 
...
\varphi_n (t, \infty) = 0 ; \quad \psi_{n+1} (t, \infty) = 0 \quad {\rm for} \; n \geq 0 .
\end{equation}
We must also require that the gauge field satisfy a compatible boundary condition:
...
By the general principal \brian{write section above} we expect the local operators $\cA_0$ at $r = 0$ to be Koszul dual to the local operators $\cA_\infty$ at $r = \infty$. 

From Equations \eqref{eqn:bc1} and \eqref{eqn:gbc1} we can compute the gauge equivalence classes of local operators $\cA_\infty$ at $r = \infty$ in a very similar way as we did for $r = 0$. 
The algebra is freely generated by the even local operators $\{\varphi_n\}_{n < 0}$ and the odd local operators $\{\chi_n\}_{n \leq 0}$. 
In other words
\[
\cA_\infty = \bigotimes_{n < 0} {\rm S}^\bu \big(V^*_{(n)}\big) \otimes \bigotimes_{n \leq 0} \wedge^\bu \big(V_{(m)}\big)
\]
which we can clearly identify as the Koszul dual algebra to $\cA_0$. 

Its instructive to compare characters of local operators at $r = 0, \infty$. 
We have already computed the character $\chi_0(q,s)$ of the algebra $\cA_0$ above. 
...
If we just remember the {\em lowest} lying $S^1$-modes, we have see that observables at $r=0$ are simply the free algebra on the even operator $\varphi_0$, thus the subalgebra of lowest lying modes of the algebra $\cA_0$ is
\[
\cA_0^{(0)} = {\rm S}^\bu(V^*).
\]

On the other hand, at $r=\infty$ the lowest lying linear operator that survives is the {\em odd} operator $\chi_0$.
So, the subalgebra of lowest lying modes of the algebra $\cA_\infty$ is
\[
\cA_\infty^{(0)} = \wedge^\bu(V) .
\]

We recognize that $\cA_0^{(0)}$ and $\cA_\infty^{(0)}$ witness the prototypical example of Koszul duality for free algebras. 
