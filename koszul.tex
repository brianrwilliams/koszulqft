...
\brian{the Yangian is already such an example I think.}
...
Physically, if $\cA$ reprsents the algebra of local operators of a quantum mechanical system, then modules label point defects. 
...
Notice that $f$ endows $\cB$ with the structure of a dg $\cA$-module by the formula $a \cdot b \define f(a) \cdot b$, for $a \in \cA$ and $b \in \cB$, where the on the right hand side we use the associative product on $\cB$. 
...
\brian{Do you think explaining that would be helpful? We could refer to \cite{LV} for this sort of thing. It just seems a little technical for the discussion so far.
If you want to touch this up, the short definition I would give goes like this. 
``Two extensions $E,E'$ are equivalent if there is an isomorphism of $\cA$-modules $E \to E'$ which can be extended to an isomorphism of short exact sequences.''}
...
One of the key characterizing features of Koszul duality is that certain subcategories of the derived categories of dg-modules of $\cA$ and $\cA^!$ are equivalent \cite{BGG, BGSch, BGSoerg, Pos}. \footnote{Certain assumption on the dg algebra $\cA$ are required for this result to hold. 
To get an idea of the technical homological algebra that goes into this, even for examples of physical interest, we refer to \cite[Section 8]{CostelloYangian}. } 
%We can denote this category $\cC \simeq \cA-\mod \simeq \cA^!-\mod$. 
We will make a few remarks on the physical consequences of this equivalence throughout the note to help orient the reader, but it will not be our main focus. 
...
\brian{I think that's probably enough...}
...
\brian{I'd say it's the most basic instance of descent (de Rham complex on the simplest contractible manifold). 
Is there a notation you can think of that looks more familiar?}
...
If we assume that the theory is also translation invariant along $M$ then there is also an equivalence \natalie{algebra isomorphism, right? or quasi-iso?} \brian{could do either, depends on how strong you want the condition "translation invariant", that's why I was vague with `equivalence'.
Of course, for most things we do, quasi-isomorphism is best.} of algebras $\cA_x \simeq \cA_{x'}$ for $x, x' \in M$. 
...
What happens if we don't treat the bulk gauge theory classically? In perturbation theory, there is a single moduli parameter for quantizing Chern--Simons theory on $\RR^3$ for a semisimple Lie algebra $\fg$ \cite[Chapter ??]{CosRenorm}. 
...
\label{eqn:N=2desc4tw} 
\chi^{(1)} &= \psi .
\end{align}
In the BV formalism $\psi_{t \zbar} \d t \d \zbar$ can be thought of as an anti-ghost to the ghost $\chi$. 
...
Along the line, the descendants of the fields $\phi, \psi$ are given by the $\d t$ components of the one-form descendants in the full theory \eqref{eqn:N=2desc1tw}--\eqref{eqn:N=2desc2tw}. 
