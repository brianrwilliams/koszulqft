...
Physically speaking, one should think about $\cA$ as being the algebra of operators of a system of quantum mechanics.
The differential $\d$ is the BRST operator. 
...
In general, to compute the space of derived homomorphisms needs to first freely resolve the module $M$, we will see an example shortly.
...
\]
is a map of cochain complexes and induces an isomorphism on cohomology (we call such a map a quasi-isomorphism).
...
\beqn
\left(\CC[x_i,\xi_i, \eta^i] \, , \, \left[x_i \frac{\d}{\d \xi_i} + c_{ij}^k \xi_k \frac{\d}{\d \xi_i}\frac{\d}{\d \xi_j}, - \right] \right) 
\eeqn
...
\beqn
\left(\CC[\eta^i] \, , \, c_{ij}^k \eta^i \eta^j \frac{\d}{\d \eta_k}  \right) 
\eeqn

\brian{I think I messed up the above example. I need to fix it...}

In conclusion we see that the Koszul dual of the commutative dg algebra $\clie^\bu(\fg)$ is the (non dg) enveloping algebra 
\[
\clie^\bu (\fg)^! \simeq U \fg .
\]

\brian{I think we move some discussion below about gauge theory here when we define the CE complex.}
...
\[
\Sym^\bu (V^\vee) \otimes \wedge^\bu(V) = \CC[x_i, \xi^j]
\]
where $\{x_i\}$ is a linear basis for $V^\vee$ and $\{\xi_j\}$ is its dual basis.
The variable $x_i$ is even parity and $\xi_j$ is odd parity; we will furthermore take a $\ZZ$-grading where $x_i$ is degree zero and $\xi^j$ is degree $-1$. 
...
Observe that the zeroth cohomology $H^0$ of this algebra is precisely the Jacobian ring of~$W$
\[
\CC[x_i] \, / \, (\partial_i W) .
\]
...
It is an easy exercise to see that even for a more general superpotential $W$ the result still holds that $\cA^! \simeq \cA$.
