...
In particular, all of our considerations are local, and are carried out at the perturbative level. We do not account for interesting global properties of quantum field theories, such as studying the theories on manifolds of nontrivial topology, nor do we account for potentially interesting nonperturbative effects, except for brief comments in \S \ref{s:conclusions}. Clarifying any interactions between Koszul duality and these effects is a topic we leave for future work. 

A notation we use liberally is the \textit{cohomological shift}. Given a graded complex $A= \oplus_i A_i$ with differentials $d_{A}^i: A_i \rightarrow A_{i+1}$, we define the n-shifted complex $A[n]$ by the relation on the graded subspaces $A[n]_i := A_{n+i}$ and differential $d^i_{A[n]} := (-1)^n d^{i+n}_{A}$. The shift has degree $n$ and the differential has degree $1$.

A crucial point here is that our complexes are viewed in a \textit{graded-commutative} sense. This means that any two homogeneous operators $a_p, a_q$ of respective degrees $p, q$ satisfy $a_p a_q = (-1)^{pq} a_q a_p$. Under an odd-degree shift, therefore, the statistics of a graded subspace will change from `bosonic' to `fermionic', and vice versa. 
...
\sum_{n \geq 0} \int_{\RR \times \{z=0\}} \frac{1}{n!} J_a [n] \partial_z^n A^a_t + \sum_{n \geq 0} \int_{\RR \times \{z=0\}} \frac{1}{n!} K_a[n] \partial_z^n B_t^a .
\eeqn
...
The evaluation of the relevant integral is outside the scope of these notes, but report that the form of the gauge anomaly is proportional to
\[
\hbar \int_{\RR \times \{z=0\}} \partial_z A_t^a \kappa^{bc} f_{ab}^d J_{c} [0] K_{d} [0] + \cdots .
\]
...
To cancel this gauge anomaly, we must introduce the following {\em differential} acting on this graded algebra:
\[
\d J_a[k] = \hbar \sum_{r + s = k-1} \kappa^{bc} f_{ab}^d J_c[r] K_d[s] .
\]
...
\[
\hbar \int_{\RR \times \{z=0\}} \partial_z B_t^a \kappa^{bc} f_{ab}^d J_{c} [0] K_{d} [0] + \cdots .
\]
...
\[
\d K_a[k] = \hbar \sum_{r + s = k-1} \kappa^{bc} f_{ab}^d f K_c[r] K_d[s] .
\]
If $k=0$ then $\d K_a [0] = 0$. 
...
\brian{I switched to using $\hbar$ as a macroscopic variable.}

In summary, we have found that in order for quantum gauge anomalies to vanish we must introduce a differential on the Koszul dual to the algebra of local operators.
This quantum dg algebra takes the form
\[
\bigg( U(\fg[\ep]) \, , \, \d \bigg) .
\]
On the $J_a[n]$ generators, $\d$ is of the form 
\[
\d J_a [n] = K_a [n-1] + \sum_{r + s = k} \kappa^{bc} f J_b[r] K_c[s] .
\]
On the $K_a[n]$ generators it is
\[
\d K_a [n] = \sum_{r + s = k} \kappa^{bc} f K_b[r] K_c[s] .
\]
