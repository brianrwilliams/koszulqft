...
Together with $Q$ we assume this equips $\cB$ with the structure of a {\em dg} algebra. 
...
Before discussing the problem of coupling a line defect inside of a higher dimensional gauge theory we discuss the coupling of two one-dimensional theories whose algebras over local operators we denote by $\cA$ and $\cB$. 
...
\brian{maybe this goes later} At the level of action functionals this corresponds to simply taking the sum of action functionals $S_{\cA} + S_{\cB}$. 

We restrict to nontrivial couplings between theories $\cA$ and $\cB$ which are visible at the level of BRST operators. 
In terms of the dg algebra of local operators this means that as a graded algebra is exactly that of \eqref{eqn:uncoupled} (the plain tensor product over $\CC$). 
...
\brian{other types of deformations, hochschild cohomology}

The addition of such an operator $\til{Q}$ would be a very general way to formulate the notion of a coupling between theories $\cA$ and $\cB$. 
We specialize the situation a bit and make the following assumption: 
\begin{itemize}
\item the operator $\til{Q}$ is {\em inner} in the sense that it is given by a pure commutator
\[
\til{Q} = [\alpha, -]_{\cA \otimes \cB}
\]
where $\alpha$ is an element in $\cA \otimes \cB$. 
%We will refer to such deformations as Hamiltonian. 
\end{itemize}
...
\brian{a infinity version}

From hereon, we refer to $\alpha$ as an {\em algebraic coupling} between two quantum mechanical systems. 
...
This is the algebra encoding the local operators of a Chern--Simons type gauge theory in one-dimension. 
...
\]
The operator $\til{Q} = \cO^{(0)} + \cO^{(1)}$ is closed in $\Omega^\bu(\RR \, , \, \cB)$ if and only if the following descent equations are satisfied 
\begin{align*}
Q \cO^{(0)} & = 0 \\
\d \cO^{(0)} + Q \cO^{(1)} & = 0 .
\end{align*}
If $\cO = \cO^{(0)}$ is of ghost number $k$ then $\cO^{(1)}$ is of ghost number $k-1$ but it also carries differential form degree one. 
So, with respect to the totalized grading on $\Omega^\bu(S, \cB)$ the operator $\cB$ does have a homogenous degree.
...
\]
where $d = \partial_{\bar{z}} d\bar{z} + \partial_t dt$ (which, in the presence of gauge fields, must be covariantized). Explicitly, $\mathcal{O}^{(1)} = {i \over 2}Q_+ \mathcal{O} d\bar{z} - i Q_- \mathcal{O} dt$. 
...
\]
which, after integration by parts, can be written more compactly in the twisted theory as\footnote{Although $\phi^{(1)}$ does not appear in the action, it plays an interesting role in understanding higher operations (brackets); see \cite{CDG} for details.} \textcolor{red}{right? confused about minus signs here...}
...
We turn to an interpretation of Koszul duality in terms of line operators of the three-dimensional theory. 
...
The most general coupling takes the form
\[
\sum_{\ell \geq 0} \int_{\{z=0\}} \frac{1}{\ell!} K_t^a [\ell] \partial_z^\ell \phi_{a} + \sum_{k \geq 0} \int_{\{z=0\}} \frac{1}{k!} J_a[k] \partial_z^k \psi_t^a  .
\] 
For some locally defined currents $J_a [k], K^a [\ell] = K_t^a [\ell] \d t$ along $\RR_t$. 

Notice that only the $\d t$-component of the one-form $\psi$ may couple to a field along the defect. 
Also, observe that while the one-form field $\psi_t$ couples to a scalar $J_a [k]$ along $\RR_t$ the field $\phi$ couples to a one-form $K^a[\ell] = K^a_t [\ell] \d t$ along $\RR_t$. 
